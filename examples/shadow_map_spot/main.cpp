...
//		float angle = glm::asin(hypotenuseLen / 1.0f) * 2.0f;
	...
	void preparePipelines()
	{
		...
		shaderStages[0] = loadShader(getShadersPath() + "shadow_mapping_spot/base.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getShadersPath() + "shadow_mapping_spot/base.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);


		VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache,1,&pipelineCreateInfo,nullptr,&pipelines.base));
		//depth pipeline
		shaderStages[0] = loadShader(getShadersPath() + "shadow_mapping_spot/shadowMap.vert.spv",VK_SHADER_STAGE_VERTEX_BIT);
